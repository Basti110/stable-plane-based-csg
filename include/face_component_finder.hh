...
public:
    ...
    FaceComponentFinder(PlaneMesh& planeMesh, const pm::edge_attribute<bool>& intersectionMark) : mPlaneMesh(planeMesh) {
        ...
        for (pm::face_handle& face : mesh.faces()) {
            if (mfaceToComponent[face] != -1)
                continue;
    ...
    pm::face_attribute<tg::color3> getColorAssignment() {
        pm::Mesh& mesh = mPlaneMesh.mesh();
        pm::face_attribute<tg::color3> colors = mesh.faces().map([this](pm::face_handle f) { 
            int component = mfaceToComponent[f];
            int colorIndex = ((component * 255) / mCountComponents);
            unsigned char r = this->turbo_srgb_bytes[colorIndex][0];
            unsigned char g = this->turbo_srgb_bytes[colorIndex][1];
            unsigned char b = this->turbo_srgb_bytes[colorIndex][2];
...
private: 
    ...
    std::vector<std::vector<pm::face_index>> mComponentToFace;
    pm::face_attribute<int> mfaceToComponent;
...
using SharedFaceComponents = std::shared_ptr<FaceComponentFinder>;